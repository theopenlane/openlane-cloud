...
// NewProviderWithRefresh function is a constructor function that creates a new instance of ConfigProviderWithRefresh
...
func (s *ProviderWithRefresh) refreshConfig() {
	for {
		...
		if err != nil {
			log.Error().Err(err).Msg("failed to load new server configuration")
			continue
		}
